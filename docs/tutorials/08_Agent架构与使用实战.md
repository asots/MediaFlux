...
> 核对日期：2026-09-10。本文描述当前 `main` 的实现，不是未来架构提案。完整工具名称、风险等级和 Provider 操作见 [Agent 工具与能力参考](09_Agent工具与能力参考.md)。
>
...
- 复杂查询应优先使用批量能力，例如 `library.batch_presence`，而不是对几十部作品逐个搜索。
...
**确认阶段不调用模型，也不接受模型追加新对象。** 即使领域元数据把一个操作标为 `LOW_WRITE`，进入 Kernel 后仍是需要确认的 WRITE，不是免确认写入。
...
| 工具次数超限 | 缩小范围，使用批量核对/分页/后台任务能力；不要单纯调大上下文窗口 |
